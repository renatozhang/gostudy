...
func GetArticleRecordListById(categoryId, page_num, page_size int) (articleRecordList []*model.ArticleRecord, err error) {
	//1.从数据库中，获取文章列表
	articleInfoList, err := db.GetArticleListByCategoryId(categoryId, page_num, page_size)
	if err != nil {
		fmt.Printf("1 get article list failed, err:%v\n", err)
		return
	}
