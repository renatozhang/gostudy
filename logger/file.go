...
// 2022/9/1 10:05:03.353 DEBUG logDeBug.go:29 this is a debug log
...
func (f *FileLogger) splitFileHour(warnFile bool) {
	if f.logSplitType == LogSplitTypeHour {
		...
		var filename string
		if warnFile {
			backupFilename = fmt.Sprintf("%s/%s.log.wf%04d%02d%02d%2d", f.logPath, f.logName, now.Year(), now.Month(), now.Day(), f.lastSplitHour)
			filename = fmt.Sprintf("%s/%s.log.wf", f.logPath, f.logName)
		} else {
			backupFilename = fmt.Sprintf("%s/%s.log%04d%02d%02d%2d", f.logPath, f.logName, now.Year(), now.Month(), now.Day(), f.lastSplitHour)
			filename = fmt.Sprintf("%s/%s.log", f.logPath, f.logName)
		}
		file := f.file
		if warnFile {
...
func (f *FileLogger) splitFileSize(warnFile bool) {
	...
	now := time.Now()
	if warnFile {
		backupFilename = fmt.Sprintf("%s/%s.log.wf%04d%02d%02d%02d%2d%02d", f.logPath, f.logName, now.Year(), now.Month(), now.Day(), now.Hour(), now.Minute(), now.Second())
		filename = fmt.Sprintf("%s/%s.log.wf", f.logPath, f.logName)
	} else {
		backupFilename = fmt.Sprintf("%s/%s.log%04d%02d%02d%02d%2d%02d", f.logPath, f.logName, now.Year(), now.Month(), now.Day(), now.Hour(), now.Minute(), now.Second())
		filename = fmt.Sprintf("%s/%s.log", f.logPath, f.logName)
	}
	file.Close()
	os.Rename(filename, backupFilename)
